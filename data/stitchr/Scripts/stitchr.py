...
import tcr_structure.stitchr.Scripts.functions as fxn
import argparse
import sys
import warnings
...
def args():
    ...
    parser.add_argument('-v', '--v', required=False, type=str, default='', help="V gene name. Required. "
                                                                                "Specific allele not required, will default to prototypical (*01)")

    parser.add_argument('-j', '--j', required=False, type=str, default='', help="J gene name. Required. "
                                                                                "Specific allele not required, will default to prototypical (*01)")

    parser.add_argument('-cdr3', '--cdr3', required=False, type=str, default='', help="CDR3 amino acid sequence. "
    ...
    parser.add_argument('-s', '--species', required=False, type=str, default='HUMAN', help="Species (common name). "
                                                                                           "Optional: see HLA_data directory for all possible options. Default = HUMAN")
...
def stitch(specific_args, tcr_info, functionality, partial_info, codon_dict, j_warning_threshold, preferences):
    ...
    :param tcr_info: sequence HLA_data for the alleles of a specific locus read in from IMGT HLA_data
    :param functionality: predicted functionality of different TCR genes, as according to IMGT
    :param partial_info: genes filtered out from input TCR HLA_data on account of being incomplete in the database
    ...
    for r in fxn.regions:
        ...
        # First check whether the gene exists in the IMGT HLA_data for this species
        if in_gene in tcr_info[fxn.regions[r]]:
            ...
            raise ValueError("Error: a " + fxn.regions[r] + " sequence region has not been found for gene " + gene +
                             " in the IMGT HLA_data for this chain/species. Please check your TCR and species HLA_data. ")
            ...
            if in_allele in tcr_info[fxn.regions[r]][gene]:
                ...
                if partial_info[gene][allele]:
                    warnings.warn("Cannot use " + gene + "*" + in_allele + " " + fxn.regions[r].lower() + "sequence, "
                                                                                                          "as it is classed as '" +
                                  partial_info[gene][in_allele] + "' (partial). ")
                else:
                    allele = in_allele

            else:
                warnings.warn("Cannot find the sequence of the requested allele, " + gene + "*" + in_allele +
                              ", for the " + fxn.regions[r].lower() + " sequence in the input FASTA HLA_data. ")
        ...
        if gene and allele:
            ...
            raise ValueError(
                "Cannot find TCR sequence HLA_data for " + r.upper() + " gene: " + in_gene + '*' + in_allele + ". ")
    ...
    # First test.py eligibility for seamless integration
    ...
    # Otherwise run regular amino-acid based germline determination
        ...
        # Get codon HLA_data, and use to check that there's no unexpected characters in the CDR3
        ...
        if specific_args['cdr3'][-1] != j_residues[used_alleles['j']]:
            warnings.warn("CDR3 provided does not end.csv with the expected residue for this J gene (" +
        ...
        # Start with 4 residues chunks, move from end.csv of V gene up to 10 residues in (very generous deletion allowance)
...
def pre_full_seq(cdr3, v, j, type="HUMAN"):
    # Get input arguments, determine the TCR chain in use, get codon table, then load the IMGT HLA_data in
    fxn.check_scripts_dir()
    input_args = args()
    input_args.cdr3 = cdr3
    input_args.v = v
    input_args.j = j
    input_args.species = type
    input_args, chain = fxn.sort_input(vars(input_args))
...
if __name__ == '__main__':
    # re = pre_full_seq("CASGGGGTLYF", "TRBV13-2", "TRBJ2-4")
    # CAASRPPNFGNEKLTF,TRAV13-01,TRAJ48-01*01
    # re = pre_full_seq("CATFDNTEVFF", "TRBV13-2", "TRBJ1-1", "MOUSE")
    # ASSTEETQY,NLVPMVATV,HLA-A*02:01,TCRBV19,TCRBJ2-5
    # ATYNNNDMR,TCRAV24,TCRAJ43
    # AGSQTQLWETQY,NLVPMVATV,HLA-A*02:01,TCRBV7-2,TCRBJ2-5
    v = it.tr.standardise("TCRBV7-2")
    j = it.tr.standardise("TCRBJ2-5")
    re = pre_full_seq("AGSQTQLWETQYF", v, j)
    print(re)
